      ...
      <div class="line">
        <btn-group>
          <hor-list :primary="app.theme.primary" :secondary="app.theme.strong">
            <gen-btn-flat @click="changeFilter('onTop')"
                          :value="filter.indexOf('onTop') > -1"
                          primary="rgba(128,128,128,0.5)" :secondary="app.theme.primary" l>
            ...
            <transition name="show-hide">
              <btn-group v-if="app.login">
                <gen-btn-flat @click="changeFilter('private')"
                              :value="filter.indexOf('private') > -1"
                              primary="rgba(128,128,128,0.5)"
                              :secondary="app.theme.primary" l>
                  <span class="note-toolbar-icon iconfont icon-lock"></span>
                  <span>{{$t('message.router.note.private')}}</span>
                </gen-btn-flat>
                <gen-btn-flat @click="changeFilter('public')"
                              :value="filter.indexOf('public') > -1"
                              primary="rgba(128,128,128,0.5)"
                              :secondary="app.theme.primary" l>
                  <span class="note-toolbar-icon iconfont icon-share"></span>
                  <span>{{$t('message.router.note.public')}}</span>
                </gen-btn-flat>
                <gen-btn-flat @click="changeFilter('collection')"
                              :value="filter.indexOf('collection') > -1"
                              primary="rgba(128,128,128,0.5)"
                              :secondary="app.theme.primary" l>
                  <span class="note-toolbar-icon iconfont icon-fabulous"></span>
                  <span>{{$t('message.router.note.collection')}}</span>
                </gen-btn-flat>
                <gen-btn-flat @click="changeFilter('working')"
                              :value="filter.indexOf('working') > -1"
                              primary="rgba(128,128,128,0.5)"
                              :secondary="app.theme.primary" l>
          ...
          <div class="search-container">
            <gen-text-box v-model:value="search"
                          @value="$emit('update:search', $event)"
                          width="10rem"
                          :primary="app.theme.primary"
                          :secondary="app.theme.strong"
                          flat l></gen-text-box>
            <gen-btn-flat :primary="app.theme.secondary"
                          :secondary="app.theme.primary"
                          transparent square l>
              <span class="iconfont icon-search"></span>
            </gen-btn-flat>
          </div>
          <transition name="show-hide">
            <gen-btn-flat v-if="app.login" @click="newNote"
  ...
  <card primary="none"
    ...
    <template v-slot:main>
      <div class="filter-line">
        <span :style="{color: app.theme.primary}"
              class="note-toolbar-icon iconfont icon-filter"></span>
        <hor-list :primary="app.theme.primary"
                  :secondary="app.theme.strong">
...
export default {
  ...
  methods: {
    ...
    newNote() {
      this.$router.push({name: 'Author/NoteEditor'})
    }
  }
}
</script>
